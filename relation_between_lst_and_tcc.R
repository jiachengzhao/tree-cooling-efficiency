## ----------------------------------
# Title: Relation between land surface temperature (LST) and tree canopy cover (TCC)
# Objective: Plotting the relation between LST and TCC
...
scatters = function(
  tree, lst,
  xlabel = 'TCC (%)',
  ylabel = expression('LST (' * degree * 'C)'),
  main, city.name
) {
  # data
  value.tree = getValues(tree)
  value.lst = getValues(lst)
  d = data.table(x = value.tree, y = value.lst)
  ...
  # add title and r2
  addequation = function(nfit) {
    eq = substitute(
      italic('y') ~ '=' ~ a * italic('x') ^ b,
      list(
        a = sprintf(
          '%.2f',
          as.numeric(coef(nfit))[1]
        ),
        b = sprintf(
          '%.2f',
          as.numeric(coef(nfit))[2]
        )
      )
    )
    return(eq)
  ...
  title(city.name, adj = 0.9, line = -1, font.main = 1, col.main = 'black', cex.main = 1.1)
  title(addequation(fit), adj = 0.9, line = -2, col.main = 'black', cex.main = 1.1)
  title(addr2(db$y, predict(fit)), adj = 0.9, line = -3, col.main = 'black', cex.main = 1.1)
}
