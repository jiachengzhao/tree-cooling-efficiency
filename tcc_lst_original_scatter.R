# Title: TCE scatter plots 
# Objective: Process TCE images to display the scatters between land surface temperature (LST) and tree canopy cover 
...
# s = stack('C:/Users/jzhao/Documents/Nutstore/Projects/Global urban afforestation cools surface up to 1 °C from 2000 to 2015/TCE scatter images/Other climates/Quebec.tif')
# writeRaster(stack(s[[5]], s[[6]]), 'pppppppppppppppppp.tif')
# par(
#   cex.axis = 1, family = 'sans', las = 1,
#   mai = c(0.1, 0.1, 0.1, 0.1), mfcol = c(4, 5), oma = c(2.5, 2.5, 2, 0.2), tck = 0.025
# )
# for (i in seq(2, nlayers(s), 2)) {
#   scatters(
#     s[[i]], s[[i - 1]], main = '', city.name = ''
#   )
# }
# par(opar)
...
scatters <- function(
  ...
  # data
  value.tree <- getValues(tree)
  value.lst <- getValues(lst)
  ...
  lines(
    ...
    predict(fit, newdata = data.table(bin = seq(1, max(db$bin), 0.01))),
    col = 'red', lwd = 1.7
  )
